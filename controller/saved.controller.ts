...
export const saved = async (req: Request, res: Response) => {
  ...
  if (!productId) return res.status(400).json({ message: "Product ID kerak" });

  const existing = await Saved.findOne({ where: { userId: user.id, productId } });
  if (existing) return res.status(400).json({ message: "Mahsulot allaqachon saved" });
...
export const removeSaved = async (req: Request, res: Response) => {
  ...
  if (!deleted) return res.status(404).json({ message: "Saved mahsulot topilmadi" });
